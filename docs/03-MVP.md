...
| **Informs** | Architecture · API · AI · Extension · Types · Roadmap |
...
- Anything in PRD marked P0 but deferred here (save, account) is **out of MVP**.
...
| M5 | Analysis is ephemeral: no login, no permanent content store |
...
- Synchronous `POST /analyze` API
- Shared Zod Trust Report types (`packages/types`, defined in `09-AI.md`)
- Provider-agnostic LLM structured output (`packages/ai`)
...
| Category | Examples |
| --- | --- |
| Identity | Login, accounts, OAuth, browser sync |
| Commerce | Payment, plans, trials, billing |
| Persistence | Analysis history, cloud save, local archive of full page bodies |
...
| Infra premature | Redis/BullMQ queues (add when latency/rate limits force it) |
| Web product | Full web app for saves/settings — landing page only if needed for install/docs |
...
User clicks Analyze
  ...
  → Mozilla Readability
  → Clean HTML → Markdown
  → POST /analyze (url, title, markdown, requestId, …)
  → Backend normalize / truncate
  → Deterministic meta signals (author/source hints from markup when available)
  → LLM structured Trust Report
  → Policy filter + Zod validate
  → Extension renders one Trust Report panel
  → Session ends; content not permanently stored
```
...
- Offline → clear offline error; no pretend success.
...
| ID | Bar |
| --- | --- |
| LAT-1 | P50 time-to-first-useful-result ≤ 5s for typical article (target) |
| LAT-2 | P95 time-to-complete-analysis ≤ 15s for typical article |
| LAT-3 | Hard backend timeout; calm retry; never fabricate a complete report on timeout |
| PRIV-1 | Ephemeral by default; **no** permanent storage of analyzed page bodies |
| PRIV-2 | Logs minimize PII and raw body content |
| PRIV-3 | Telemetry excludes raw page body by default |
| SEC-1 | Treat page content as untrusted (prompt-injection resistant) |
| SEC-2 | TLS in transit; secrets not in client |
| SEC-3 | Least-privilege extension permissions (`activeTab` / on-demand scripting preferred) |
| SAFE-1 | Automated check for banned verdict phrases on user-facing outputs |
| REL-1 | Idempotent analyze via `requestId` |
| REL-2 | Rate limit anonymous analyze (IP / extension key) |
| A11Y-1 | Panel keyboard-navigable; do not encode meaning by color alone |

Redis/BullMQ: **not required** for MVP. Use a synchronous NestJS path; introduce a queue only if measurement shows need.
...
| Topic | MVP rule |
| --- | --- |
| Default | Analyze → report → discard |
| Backend | May process in memory / short request lifecycle; must not persist full content as product history |
| Auth | None |
| Save / delete / history | Deferred (see §10) |
| Sensitive content | Ephemeral path is the product promise; no “save” footgun in MVP |

This satisfies Vision T5 and Golden Rule 4 for the MVP cut: we do not permanently store analyzed content because the user cannot opt into save yet — and we do not add save until product+privacy UX is ready.

---

# 10. Explicit PRD deltas (deferred)

These appear as MVP/P0 in `02-PRD.md` but are **deferred** by this cut:

| PRD item | Deferral |
| --- | --- |
| G6 Ephemeral + explicit save/delete | Ephemeral only; save/delete post-MVP |
| F07 Save Analysis | Deferred |
| F08 Delete / Manage Saved | Deferred |
| F09 Ephemeral Mode Default | Kept as sole mode (always ephemeral) |
| F14 Optional Account | Deferred |
| F15 Settings | Minimal local-only if needed later; not a product surface now |
| F16 Saved History | Deferred |
| G9 Account + sync | Deferred |
| Queue / workers (NFR-S3/S4) | Deferred until forced by scale |
| Selection analyze (F11), Copy pack (F10) | Deferred P1 |

Agents must not “helpfully” reintroduce save/login to satisfy PRD numbering.
...
3. `packages/ai` provider-agnostic structured output
4. `apps/api` `POST /analyze`
5. `apps/extension` extract → analyze → Trust Report UI
6. Minimal `apps/web` landing (install / explain product) when needed

---

# Closing

MVP success is a calm, explainable Trust Report in the browser — not a larger feature list.
